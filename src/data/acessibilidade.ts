  ...
  [
    {
      ...
      texto: {
        pt: 'O axe-core corre sobre as sete rotas publicadas, nos dois temas, com a árvore fechada e aberta — vinte e oito combinações — antes de cada publicação, e bloqueia-a se encontrar alguma coisa. O contraste é calculado a partir dos tokens de cor, a árvore inteira é aberta e fechada sem JavaScript, as legendas dos vídeos são conferidas contra a duração medida de cada um, e cada uma das ligações que saem do sítio é contada e obrigada a avisar que abre uma janela nova — na seta, para quem a vê, e no nome acessível, para quem não a vê.',
        en: 'axe-core runs over the seven published routes, in both themes, with the tree closed and open — twenty-eight combinations — before every release, and blocks it if anything turns up. Contrast is computed from the colour tokens, the whole tree is opened and closed with JavaScript off, the video captions are checked against each film’s measured length, and every single link that leaves the site is counted and made to warn that it opens a new window — in the arrow, for those who see it, and in the accessible name, for those who do not.',
        fr: 'axe-core parcourt les sept routes publiées, dans les deux thèmes, l’arbre fermé puis ouvert — vingt-huit combinaisons — avant chaque publication, et la bloque au moindre problème. Le contraste est calculé à partir des jetons de couleur, l’arbre entier s’ouvre et se ferme sans JavaScript, les sous-titres sont vérifiés contre la durée mesurée de chaque vidéo, et chacun des liens qui sortent du site est compté et tenu d’annoncer qu’il ouvre une nouvelle fenêtre — par la flèche, pour qui la voit, et par le nom accessible, pour qui ne la voit pas.',
        es: 'axe-core recorre las siete rutas publicadas, en los dos temas, con el árbol cerrado y abierto — veintiocho combinaciones — antes de cada publicación, y la bloquea si encuentra algo. El contraste se calcula a partir de los tokens de color, el árbol entero se abre y se cierra sin JavaScript, los subtítulos se comprueban contra la duración medida de cada vídeo, y cada uno de los enlaces que salen del sitio se cuenta y se obliga a avisar de que abre una ventana nueva — en la flecha, para quien la ve, y en el nombre accesible, para quien no.',
      },
    },
    {
